    ...
    return(
        ...
        className="flex justify-center items-center flex-col px-8 py-14 sm:flex-row lg:px-16" id="contactus">
            <motion.div variants={buttonVariant(0.5)} className="flex-1 mr-4">
                ...
                <input className="bg-slate-400 placeholder-white font-bold rounded-xl px-4 h-[40px] w-[100%]" value={formcontact.name} onChange={(event) =>
    setFormcontact((prev) => ({ ...prev, name: event.target.value }))
  }placeholder="Name..."/>
                <input className="bg-slate-400 placeholder-white font-bold rounded-xl px-4 h-[40px] my-8 w-[100%]" value={formcontact.email} onChange={(event) =>
    setFormcontact((prev) => ({ ...prev, email: event.target.value }))
  } placeholder="Email..."/>
                <textarea className="bg-slate-400 placeholder-white font-bold mb-10 rounded-xl px-4 w-[100%] py-4" value={formcontact.message} onChange={(event) =>
    setFormcontact((prev) => ({ ...prev, message: event.target.value }))
  } placeholder="Message..."></textarea>
                <button onClick={handlesubmit} className="bg-blue-200 hover:bg-blue-300 text-white font-bold py-2 px-4 rounded">
  submit
</button>
            </motion.div>
            <motion.div variants={buttonVariant(0.5)} className="flex-1 mr-4">
